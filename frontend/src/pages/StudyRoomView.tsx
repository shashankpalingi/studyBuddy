...
const StudyRoomView: React.FC = () => {
  ...
  const [lastActiveYoutubeState, setLastActiveYoutubeState] =
    ...
    const fetchRoomData = async () => {
      ...
      try {
        ...
        if (!roomData.participants.includes(currentUser.uid)) {
          console.log(
            `User ${currentUser.uid} not in participants for room ${roomId}. Adding...`,
          );
          ...
          if (roomData.isPrivate) {
            ...
            setError(
              "This is a private room. Please join using the correct join code from the Study Rooms page.",
            );
            setIsLoading(false);
            return;
          }
        ...
        // Find creator
        const creatorData = participantsData.find(
          (p) => p.uid === roomData.createdBy,
        );
        if (creatorData) {
  ...
  const handleLeaveRoom = async () => {
    ...
    if (room.createdBy === currentUser.uid) {
      setError(
        "As the room creator, you cannot leave. You must close the room instead.",
      );
      return;
    }
    ...
    try {
      ...
      setError(
        error.message || "Failed to leave study room. Please try again.",
      );
    } finally {
      setIsLeaving(false);
    }
  ...
  const handleCloseRoom = async () => {
    ...
    try {
      ...
      setError(
        error.message || "Failed to close study room. Please try again.",
      );
    } finally {
      setIsClosing(false);
    }
  ...
  const handleDeleteRoom = async () => {
    ...
    if (
      ...
      try {
        ...
        setError(
          error.message || "Failed to delete study room. Please try again.",
        );
      } finally {
        setIsDeleting(false);
      }
  ...
  useEffect(() => {
    if (roomId) {
      ...
      const savedFeature = localStorage.getItem(
        `studyroom-${roomId}-activeFeature`,
      );
    ...
    return (
      <div className="study-room-view-container">
        <div className="error-state">
          ...
          <button className="back-btn" onClick={() => navigate("/study-rooms")}>
            Back to Study Rooms
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#1a1a1a]">
      ...
      <div className="studyroom-nav">
        ...
        <div className="nav-center">
          <span className="active-feature-title">
            {activeFeature === "aichat"
              ? "AI Chat Assistant"
              : activeFeature === "codingide"
                ? "Coding IDE"
                : activeFeature === "pdfs"
                  ? "PDF Discussion"
        ...
        <div className="nav-right">
          ...
          <div className="quick-access-tools">
            ...
            <div
              className={`compact-timer ${timerExpanded ? "expanded" : ""} ${timerState.isRunning ? "running" : ""} mode-${timerState.mode}`}
            >
              ...
              {timerExpanded && (
                <div
                  className={`timer-expanded-controls mode-${timerState.mode}`}
                >
        ...
        <div className={`sidebar ${sidebarMinimized ? "minimized" : ""}`}>
          ...
          <div className="sidebar-nav">
            ...
            >
              <span className="icon">📝</span>
              {!sidebarMinimized && (
                <span className="label">Collaborative Notes</span>
              )}
            </button>
            ...
            <button
              onClick={() => changeActiveFeature("aichat")}
              className={`sidebar-btn ${activeFeature === "aichat" ? "active" : ""}`}
              title="AI Chat Assistant"
            >
              <span className="icon">🤖</span>
              {!sidebarMinimized && (
                <span className="label">AI Chat Assistant</span>
              )}
            </button>
          ...
          <div
            className={`sidebar-footer ${sidebarMinimized ? "minimized" : ""}`}
          >
            {isCreator ? (
              <button
